...
! 2016-04-21 [SKC]: Add ability to write a diagnostic, and read in data from a file.
! read_data from fms_mod
! register_diag_field and send_data from diag_manager_mod

use             fms_mod, only: FATAL, close_file, mpp_pe, mpp_root_pe, write_version_number, read_data
...
use spectral_dynamics_mod, only: get_axis_id ! SKC
use time_manager_mod, only: time_type ! SKC
use transforms_mod, only: grid_domain ! SKC
...
logical :: load_crude_land       = .false. !! 2016-04-21 [SKC]: Scale evaporation at a location based on a value from a file.

! 2016-04-21 [SKC]: Add namelist parameter (boolean) to control whether we damp the evaporation using values from a file. load_crude_land
...
! 2016-04-21 [SKC]: Add diag id, and array allocation for crude_land, used
integer :: id_crude_land ! fraction to reduce evaporation by (1 means bring evaporation to 0, 0 means full evaporation)
real, allocatable, dimension(:,:) :: crude_land ! crude land
...
subroutine surface_flux_1d (                                           &
     ...
     dt,        land,      seawater,     avail, Time, j0, is, ie, js, je) ! 2016-04-21 [SKC]: Added Time argument, j0
  ! ---- arguments -----------------------------------------------------------
  ...
  type(time_type), intent(in) :: Time ! 2016-04-21 [SKC]: Add Time argument
  integer, intent(in) :: j0, is, ie, js, je ! SKC
  ! ---- local constants -----------------------------------------------------
  ...
  if (do_init) call surface_flux_init(Time, is, ie, js, je)
...
!     thv_surf= t_surf0                        ! surface virtual (potential) T -- just for testing tun off the q_surf
  ...
  where (avail)
     ...
     flux_q = flux_q * (1.0 - crude_land(:,j0)) ! 2016-04-21 [SKC]: Scale down evaporation at specified locations
...
! 2016-04-21 [SKC]: Add land diagnostic
if(id_crude_land > 0) used = send_data(id_crude_land, crude_land, Time)
!
...
subroutine surface_flux_0d (                                                 &
     ...
     dt,          land_0,       seawater_0,  avail_0, Time, j0, is, ie, js, je)
  ...
  call surface_flux_1d (                                                 &
       ...
       dhdt_surf, dedt_surf,  dedq_surf,  drdt_surf,                     &
       dhdt_atm,  dedq_atm,   dtaudu_atm, dtaudv_atm,                    &
       dt,        land,      seawater, avail, Time, j0, is, ie, js, je) ! 2016-04-21 [SKC]: Add Time argument
...
subroutine surface_flux_2d (                                           &
     ...
     dhdt_surf, dedt_surf,  dedq_surf,  drdt_surf,                     &
     dhdt_atm,  dedq_atm,   dtaudu_atm, dtaudv_atm,                    &
     dt,        land,       seawater,  avail, Time, j0, is, ie, js, je)  ! SKC
  ...
  real, intent(in) :: dt !< Time step (it is not used presently)
  type(time_type), intent(in) :: Time ! 2016-04-21 [SKC]: Add Time argument
  integer, intent(in) :: j0, is, ie, js, je ! 2016-04-21 [SKC]: I think only surface_flux_2d is ever called -- we need some way to tell surface_flux_1d
      ! What longitude we are at.  We'll try this...
  ...
  do j = 1, size(t_atm,2)
     call surface_flux_1d (                                           &
          ...
          dt,             land(:,j),       seawater(:,j),  avail(:,j), Time, j, is, ie, js, je) ! SKC
  end do
...
subroutine surface_flux_init(Time, is, ie, js, je)

  type(time_type), intent(in) :: Time ! 2016-04-21 [SKC]: Add Time argument
  integer, intent(in) :: is, ie, js, je ! SKC
...
allocate(crude_land (is:ie, js:je)) ! SKC
...
! 2016-04-21 [SKC]: Add diag field for crude_land
axes = get_axis_id()
id_crude_land = register_diag_field('surface_flux', 'crude_land', axes(1:2), Time, 'crude land', 'fraction')
!

! 2016-04-21 [SKC]: Read in crude_land if toggled on; otherwise set to zero.
if (load_crude_land) then
      call read_data('INPUT/crude_land.nc', 'crude_land', crude_land) !, grid_domain)
else
      crude_land = 0.0
endif
