...
logical :: fixed_h2o_rad = .false.

namelist / idealized_moist_phys_nml / turb, lwet_convection, roughness_heat, radiation_scheme, mixed_layer_bc, &
                                      roughness_moist, roughness_mom, do_virtual, fixed_h2o_rad ! 2016-02-14 [SKC]: Add fixed_h2o_rad nml parameter
...
integer ::           &
     ...
     id_netrad_toa                ! net radiation at top of atmosphere. I think this is correct.
...
id_net_surf_sw_down   = register_diag_field(mod_name, 'sw_down_sfc', axes(1:2), Time, 'SW flux down at surface', 'watts/m^2')
...
subroutine idealized_moist_phys(Time, p_half, p_full, z_half, z_full, ug, vg, tg, grid_tracers, &
                                previous, current, dt_ug, dt_vg, dt_tg, dt_tracers, fixed_sphum) ! 2016-01-30 [SKC]: Added fixed_sphum argument

type(time_type),            intent(in)    :: Time
real, dimension(:,:,:,:),   intent(in)    :: p_half, p_full, z_half, z_full, ug, vg, tg
real, dimension(:,:,:),     intent(in)    :: fixed_sphum ! 2016-01-30 [SKC]: Added fixed_sphum argument
...
if(rad_scheme == TWO_STREAM_RADIATION) then
   ...
   dt_tg_tmp = dt_tg
   ! 2016-02-14 [SKC]: Add option to use fixed or dynamic sphum in radiation code.
   if (fixed_h2o_rad) then
      call full_radiation_driver(Time, Time_step, p_half(:,:,:,current), p_full(:,:,:,current), tg(:,:,:,current), &
                                 fixed_sphum, z_half(:,:,:,current), z_full(:,:,:,current), dt_tg, &
                                 tdtlw, tdtsw, net_surf_sw_down, surf_lw_down) ! 2016-01-30 [SKC]: Changed grid_tracers(:,:,:,current,nsphum) to fixed_sphum
   else
      call full_radiation_driver(Time, Time_step, p_half(:,:,:,current), p_full(:,:,:,current), tg(:,:,:,current), &
                                 grid_tracers(:,:,:,current,nsphum), z_half(:,:,:,current), z_full(:,:,:,current), dt_tg, &
                                 tdtlw, tdtsw, net_surf_sw_down, surf_lw_down)
   end if
   ! 2016-02-14 [SKC]: End option logic
end if
             ...
             p_half(:,:,num_levels+1,current),                              &
                             ...
                             rough_moist(:,:),                              &
                              ...
                              dtaudv_atm(:,:),                              & ! is intent(out)
                                    ...
                                    land(:,:),                              &
                               .not.land(:,:),                              &
                                   avail(:,:),  Time, 0, is, ie, js, je) ! 2016-04-21 [SKC]: Add Time and j0, is, ie, js, je
