...
use                  fms_mod, only: set_domain, write_version_number, field_size, file_exist, &
                                    mpp_pe, mpp_root_pe, error_mesg, FATAL, WARNING, read_data, write_data, nullify_domain
...
real, allocatable, dimension(:,:,:) :: fixed_sphum ! 2016-01-29 [SKC]: Added fixed_sphum variable
...
! 2016-01-29 [SKC]: Add a fixed sphum variable to fill from climatology file upon init.
allocate (fixed_sphum (is:ie, js:je, num_levels))
!--------------------------------------------------------------------------------------------------------------------------------
file = 'INPUT/fixed_h2o.nc' ! 2016-01-29 [SKC]: Add new file read-in statement.
if(file_exist(trim(file))) then
  call get_lon_max(lon_max)
  call get_lat_max(lat_max)
  call field_size(trim(file), 'sphum', siz)
  if(lon_max /= siz(1) .or. lat_max /= siz(2)) then
    write(message,*) 'Resolution of H2O data does not match resolution specified on namelist. H2O data: lon_max=', &
                     siz(1),', lat_max=',siz(2),'  Namelist: lon_max=',lon_max,', lat_max=',lat_max
    call error_mesg('atmosphere_init', message, FATAL)
  endif
  call read_data(trim(file), 'sphum', fixed_sphum(:,:,:), grid_domain)
endif
!--------------------------------------------------------------------------------------------------------------------------------
...
Time_next = Time + Time_step
   ...
   call idealized_moist_phys(Time, p_half, p_full, z_half, z_full, ug, vg, tg, grid_tracers, &
                             previous, current, dt_ug, dt_vg, dt_tg, dt_tracers, fixed_sphum) ! Added fixed sphum argument 2016-01-30 [SKC]
#else
...
call spectral_diagnostics(Time_next, psg(:,:,future), ug(:,:,:,future), vg(:,:,:,future), &
                          tg(:,:,:,future), wg_full, grid_tracers(:,:,:,:,:), future, nhum, fixed_sphum) ! 2016-01-29 [SKC]: Added fixed sphum argument
...
deallocate (fixed_sphum) ! 2016-01-29 [SKC]: Deallocate variable that we added.
