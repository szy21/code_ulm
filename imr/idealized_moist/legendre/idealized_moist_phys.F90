...
real, allocatable, dimension(:,:) :: &
     ...
     skc_forcing            ! 2016-02-20 [SKC]: Add ability to impose a shortwave forcing at TOA
...
allocate(skc_forcing (is:ie, js:je)) ! 2016-02-20 [SKC]: Allocate skc_forcing.
...
if(two_stream) then
   call two_stream_gray_rad_down(is, js, Time, &
                       ...
                       net_surf_sw_down(:,:),  &
                       surf_lw_down(:,:),      &
                       skc_forcing(:,:)        ) ! 2016-02-20 [SKC]: Add skc_forcing
end if
