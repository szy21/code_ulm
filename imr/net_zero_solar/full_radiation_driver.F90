...
!=================================================================================================================================
subroutine full_radiation_driver(Time, Time_step, p_half, p_full, tg, sphum, z_half, z_full, tdt, tdtlw, tdtsw, net_surf_sw_down, surf_lw_down) ! 01-25-2016 [SKC]: Add lat
...
integer :: days,seconds,time_step_int
...
!    call print_date(Time, label(1:15)//' Ozone')
...
!yim check sphum
      do k = 1, size(sphum,3)
         do j=1, size(sphum,2)
         do i=1, size(sphum,1)
           if(sphum(i,j,k) .le. 0.) then
             sphum2(i,j,k)=1.e-15
           else
             sphum2(i,j,k)=sphum(i,j,k)
           endif
         enddo
         enddo
      enddo
 ...
 if (Rad_control%do_lw_rad) then
    ...
    call sealw99(p_full, p_half, tg, t_half, sphum2, deltaz, qo3, rco2, rf11, rf12, rf113, rf22, &
...
! 03-02-2016 [SKC]: Compute shortwave forcing: Asymmetry remains -2M
m0_tropics = 0.10404437315447089 * 2.0
m0_extratropics = 0.10653171090667758 * 2.0
...
skc_forcing(:,:) = 3.1415927 * ((-1.0 * (skc_tropics_mag / m0_tropics) * exp(-(rad_lat(:,:) - mid_tropics)**2 / (2.0 * (c_tropics**2)))) &
      + ((skc_tropics_mag / m0_tropics) * exp(-(rad_lat(:,:) + mid_tropics)**2 / (2.0 * (c_tropics**2)))) &
      - ((skc_extratropics_mag / m0_extratropics) * exp(-(rad_lat(:,:) - mid_extratropics)**2 / (2.0 * (c_extratropics**2)))) &
      + ((skc_extratropics_mag / m0_extratropics) * exp(-(rad_lat(:,:) + mid_extratropics)**2 / (2.0 * (c_extratropics**2)))) &
      ) / (cos(rad_lat(:,:)))
!
 ...
 if (Rad_control%do_sw_rad) then
    ...
    call swresf(p_full, p_half, tg, sphum2, deltaz, albedo_vis_dir, albedo_nir_dir, albedo_vis_dif, albedo_nir_dif, qo3, rco2, rch4,&
                rn2o, solflxband, solar_constant, Astro_rad%rrsun, Astro_rad%cosz, Astro_rad%fracday, camtsw, cldsctsw, cldextsw,&
                cldasymmsw, aeroasymfac, aerosctopdep, aeroextopdep, Rad_control%do_totcld_forcing, flag_stoch, Sw_output(1), skc_forcing) ! 01-25-2016 [SKC]: Add skc_forcing
...
!    call print_date(Time, label(1:15)//' SW')
...
!    call print_date(Time, label(1:15)//' Diag')
