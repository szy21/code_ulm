...
! 2016-03-19 [SKC]: Add compute_qs function to compute RH diagnostic
...
integer :: id_fixed_sphum ! 2016-01-29 [SKC]: Added id_fixed_sphum
integer :: id_rh ! 2016-03-18 [SKC]: Added id_rh
...
id_fixed_sphum = register_diag_field(mod_name, &
      'fixed_sphum', axes_3d_full, Time, 'fixed specific humidity', 'unitless')

id_rh = register_diag_field(mod_name, &
      'rh',    axes_3d_full,       Time, 'relative humidity at full model levels', 'unitless')
...
!===================================================================================
subroutine spectral_diagnostics(Time, p_surf, u_grid, v_grid, t_grid, wg_full, tr_grid, time_level, nhum, fixed_sphum)

type(time_type), intent(in) :: Time
real, intent(in), dimension(is:, js:)          :: p_surf
real, intent(in), dimension(is:, js:, :)       :: u_grid, v_grid, t_grid, wg_full
real, intent(in), dimension(is:, js:, :)            :: fixed_sphum ! 2016-01-29 [SKC]: Added fixed sphum diagnostic
real, intent(in), dimension(is:, js:, :, :, :) :: tr_grid
integer, intent(in) :: time_level
integer, intent(in), optional :: nhum ! 01-18-2016 [SKC]: Needed for computing 'height' diagnostic using virtual temperature

real, dimension(is:ie, js:je, num_levels)    :: ln_p_full, p_full, z_full, worka3d, workb3d, qsat ! 2016-03-19 [SKC]: Add qsat
...
if(id_fixed_sphum > 0) used = send_data(id_fixed_sphum, fixed_sphum, Time) ! 2016-01-29 [SKC]: Added fixed sphum diagnostic
...
if(id_zfull > 0 .or. id_zhalf > 0) then
  ! 01-18-2016 [SKC]: Needed for computing 'height' diagnostic using virtual temperature
  call compute_pressures_and_heights(t_grid, p_surf, surf_geopotential, z_full, z_half, p_full, p_half, tr_grid(:,:,:,time_level,nhum))
else if(id_pres_half > 0 .or. id_pres_full > 0 .or. id_slp > 0 .or. id_rh > 0) then ! 2016-03-19 [SKC]: Add id_rh
...
! 2016-03-19 [SKC]: Add rh diagnostic
if(id_rh>0) then
   call compute_qs(t_grid, p_full, qsat)
   worka3d = tr_grid(:,:,:,time_level,nhum) / qsat
   used = send_data(id_rh, worka3d, Time)
endif
!
